...
constexpr NvMemoryConfig make_config(const NvMemoryId id) noexcept {
  switch (id) {
#define RU_POSIX_NV_MEMORY_CASE(name, offset_value, size_value) \
    case NvMemoryId::name:                                      \
      return {static_cast<uint32_t>(offset_value),              \
              static_cast<uint32_t>(size_value)};
    RU_POSIX_NV_MEMORY_MAP(RU_POSIX_NV_MEMORY_CASE)
#undef RU_POSIX_NV_MEMORY_CASE
    default:
      return {};
  }
}
...
opaque_nv_memory make_opaque(const NvMemoryId id) noexcept {
  ...
  return opaque_nv_memory{std::string{RU_POSIX_NV_MEMORY_FILE}, config.offset,
                          config.capacity};
}
}  // namespace
